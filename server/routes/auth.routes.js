...
router.post('/signUp', [
  check('name', 'Name must contain at least 2 characters').isLength({min: 2}),
  check('email', 'Email entered incorrectly').isEmail(),
  check('password', 'Password must contain a capital letter').matches(/[A-Z]/),
...
router.post('/logIn', [
  check('email', 'Email entered incorrectly').normalizeEmail().isEmail(),
